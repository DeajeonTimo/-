...
*	HashMap<String, Phone> 변수명;  *
	...
	3) 프로그램을 종료할때 Map의 데이터가 변경(추가, 수정, 삭제했을때)되었으면
		파일로 저장 후 종료되도록 한다.
실행예시)
	메  뉴
1. 전화번호 등록
...
public class PhoneBookTest {
	...
	public void startPhoneBook() {
		...
		try {
		ObjectInputStream oin = new ObjectInputStream(new BufferedInputStream(new FileInputStream("d:/d_other/phone.data")));
		Object obj=null;
		
		System.out.println("저장된 전화번호를 불러오는 중입니다.");
		
		while((obj = oin.readObject())!=null) {
			Phone p = (Phone)obj;
			phoneBookMap.put(p.getName(),p);
		}
		System.out.println("전화번호를 모두 불러왔습니다.");
	...
	private void saveNumber() {
		...
		try {
			ObjectOutputStream oout = new ObjectOutputStream(new BufferedOutputStream(new FileOutputStream("d:/d_other/phone.data")));
