...
public class RequestTest01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//클라이언트가 보낸 데이터의 문자 인코딩 방식 지정 ==> POST방식의 설정
		...
		out.println("<body>");
		out.println("<h2>Requset의 파라미터 데이터 받기</h2>");
		...
		//		==> 이 Map객체의 key값은 '파라미터명'이며 자료형은 'String'이고 value값은 해당 파라미터에 저장된 '값'이며 자료형은 String[]이다
		...
		for(String key : paraMap.keySet()) {
			...
			if(valueArr==null || valueArr.length==0) {//값이 없을 경우
				continue;
			}else if(valueArr.length==1){ //파라미터가 배열이 아닐 경우
