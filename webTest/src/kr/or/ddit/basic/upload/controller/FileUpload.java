...
/*
 	- Servlet 3.0 이상에서 파일 업로드를 하려면 서블릿에 @MultipartConfig 애노테이션을 설정해야한다.
 	
 	- @MultipartConfig 어노테이션의 설정해야 할 속성들...
 	1) location : 업로드한 파일이 임시적으로 저장될 경로 지정("기본값 : "")
 	2) fileSizeThreshold : 이 속성에 지정한 값보다 큰 파일이 전송되면 location속성에 지정한 임시 경로에 저장된다(단위 : byte, 기본값 : 0(무조건 파일로 저장))
 	3) maxFileSize : 1개 파일의 최대 크기 설정 ( 단위 : byte, 기본값 : -1(무제한))
 	4) maxRequestSize : 서버로 전송되는 Request데이터 전체의 최대 크기 설정 (모든 파일의 크기의 합 + formData) (단위 byte, 기본값 : -1(무제한))
...
public class FileUpload extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// GET 방식으로의 요청이 오면 Upload Form화면이 출력되도록 한다.
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 파일이 아닌 일반 데이터는 getParameter()매서드나 getParameterValues() 매서드를 이용해서 구한다.
		...
		/*
		 	 - 서블릿 3.0이상에서 새롭게 추가된 Upload용 매서드
		...
		for(Part part : request.getParts()) {
			...
			if(fileName!=null) {
				...
				fvo.setFile_size((long)(Math.ceil(part.getSize()/2014.0)));
	...
	//Part 구조 안에서 파일명을 찾는 매서드
	
	/*
	   - Part의 구조
	  
	  1) 파일이 아닌 일반 데이터일 경우
	  ---------------231sdkfjh23sdlk 	==> Part들을 구분한 구분선
	  ...
	  2) 파일일 경우
	  ---------------231sdkfjh23sdlk 	==> Part들을 구분한 구분선
	  content-disposition : form-data; name="upFile1"; filename="text1.txt" ==> 파일 정보
