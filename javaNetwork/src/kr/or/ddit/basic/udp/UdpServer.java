...
/*
 	...
 	DatagramSocket객체와 DatagramPacket객체를 이용하여 통신한다.
 		...
 		- DatagramPacket : 주고받는 데이터와 관련된 작업을 수행한다.(편지, 소포)
 				=> 수신용 생성자와 송신용 생성자를 따로 제공한다.
 		- TCP방식은 Stream을 이용해서 송수신하지만  UDP방식은 Datagram을 이용해서 송수신한다.
	...
	public static void main(String[] args) {
		try {
			...
			while(true) {
				...
				// 데이터 수신하기  ==>Socket객체의 receive()매서드 이용한다.
				// receive()매서드는 데이터가 올때까지 기다린다.
				...
				// 상대방이 보낸 메시지 출력하기
				// 수신된 패킷.getData() ==> 실제 읽어온 데이터를 byte형 배열로 반환한다.
				// 수신패킷을 생성할때 인수로 지정한 byte형 배열에도 데이터가 저장된다.
