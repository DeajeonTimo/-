...
public class TcpMultiChatServer {
	...
	public TcpMultiChatServer() {
		// clientMap을 동기화 처리가 되도록 Map객체를 생성한다.
	...
	//Inner Class로 서버에서 클라이언트가 보낸 메시지를 받아서  접속된 모든 클라이언트들에게 보내는 Thread 클래스 작성
	...
	class ServerReceiver extends Thread{
		...
		public void run() {
			String name = "";
			try {
				//클라이언트가 연결이 성공하면 첫번째 데이터로  '대화명'을 입력받아 전송한다.
				// 서버는 이 '대화명'이 중복되는지 여부를 검사하여
				// 클라이언트에게 응답으로 결과를 보낸다.
				
				//클라이언트가 보낸 '대화명이' 중복되지 않을때까지 반복.
				while(true) {
				...
				//접속한 사람의 대화명을 이용하여 다른 전체 클라이언트에게 대화방 참여 메세지 전송하기
				...
				//한 클라이언트가 보낸 메세지를 받아서 전체 클라이언트들에게 보내기
				while(din!=null) {
