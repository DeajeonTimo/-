...
/*
 	...
 	- DB 자료 처리순서
 	1. 드라이버 로딩 ==> JDBC 라이브러리를 사용할 수 있도록 메모리를 읽어들이는 작업
 	...
 	4. 결과 처리  ==> 질의 결과를 받아서 원하는 작업을 수행한다.
 	 	...
 	 	2) SQL문장이 'select'문이 아닐 경우(insert, update,delete 등)에는 정수값을 반환한다.
 	 			(이 정수값은 보통 실행에서 성공한 레코드 수를 말한다.)
 	 			
 	 5. 사용했던 자원 반납하기 ==> 각 객체의 close()매서드를 이용한다.
	...
	public static void main(String[] args) {
		...
		// 3.질의
			...
			String sql = "SELECT LPROD_ID,\r\n" + 
					...
					"        LPROD_NM AS NM\r\n" + 
					"FROM LPROD";
			// 3-2 Statement 객체 생성 ==> 질의하는 객체 생성 (Connection 객체를 이용하여 생성한다.)
			stmt = conn.createStatement();
			
			// 3-3 SQL문장을 DB서버로 보내서 결과를 얻어온다. (실행할 SQL문장이 'select'문이기 때문에 ResultSet객체에 저장되어 반환된다)
			rs = stmt.executeQuery(sql);
			
			// 4 결과 처리하기 ==> 한 레코드씩 화면에 출력하기. 
			// 			ResultSet 객체에 저장된 데이터를 차례로 꺼내오려면 반복문과  next()매서드를 이용하여 처리한다.
			System.out.println(" == 쿼리문 처리 결과 ==");
			
			//ResultSet객체의 next()매서드
			// ==> ResultSet객체의 데이터를 가리키는 포인터를 다음번째 레코드로 이동시키고 그곳에서 데이터가 있으면 true , 없으면 false를 반환한다.
			
			while(rs.next()) {
				// 포인터가 가리키는 곳의 자료를 가져오는 방법
				// 형식 1) ResultSet객체 .get자료형 이름("컬럼명 또는 "alias명")
				// 형식 2) ResultSet객체.get자료형 이름 (컬럼번호) ==> 컬럼번호는 1부터 시작
